...
import CurrencyAddressLogo from 'components/CurrencyAddressLogo'
...
const OptionButton = styled.div<{ enabled: boolean }>`
  ...
  align-items: center;
  background-color: ${({ theme, enabled }) => (enabled ? theme.secondary1 : 'transparent')};
  color: ${({ theme, enabled }) => (enabled ? theme.white : theme.secondary1)};
  :hover {
    opacity: 0.6;
    cursor: pointer;
  }
              ...
              .map((t, i) => {
                return (
                  <HoverRowLink onClick={() => handleNav(networkPrefix(activeNetwork) + 'tokens/' + t.address)} key={i}>
                    <ResponsiveGrid>
                      <RowFixed>
                        <CurrencyAddressLogo address={t.address} />
                        <TYPE.label ml="10px">
                ...
                return (
                  <HoverRowLink onClick={() => handleNav(networkPrefix(activeNetwork) + 'pools/' + p.address)} key={i}>
                    <ResponsiveGrid key={i}>
                      <RowFixed>
                        <DoubleCurrencyAddressLogo address0={p.token0.address} address1={p.token1.address} />
